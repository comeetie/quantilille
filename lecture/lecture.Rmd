...
plot(accidents.2019.paris %>% filter(grav ==3 ) , pch = 20,
     col = "orange", add=TRUE, cex = 1)
plot(accidents.2019.paris %>% filter(grav == 2) , pch = 20,
...
individus_tues <- accidents.2019.paris %>%
  filter(grav == 2) %>%
  mutate(age=2019-an_nais) # On ajoute leur âge
...
Ajouter une légende pour la taille des ronds proportionnels ne peut pas être fait facilement. 
...
<!-- Recopier periph.R TODO Etienne -->

L'objectif de ce traitement est de compter le nombre d'accidents par tronçons de 100m sur le périphérique. Pour commencer nous allons extraire des données osm le squelette du périphérique. Pour ce faire nous allons filtrer les data.frame `roads` à partir d'une liste de noms. Nous aurions pu nous servir d'une petite carte interactive pour trouver cette sélection.
...
periph_simple <- roads %>% 
  ...
  select(name)
plot(periph_simple %>% st_geometry(),col=1:nrow(periph_simple),lwd=4)
```

C'est un bon début, mais il reste un certains nombre de problèmes:

- les lignes ne font pas 100m
- il reste les deux voies parallèles au niveau des tunnels


Pour résoudre ceux-ci nous allons nous servir de la librairie `sfnetwork`. Cette librairie permet de marier des objets `sf` avec une description de leur topologie sous forme de graphe. Elle s'appuie sur la librairie `tidygraph` pour ce faire. Cette description va nous être bien utile pour fusioner toute les lignes qui se touchent en une grande ligne. Commençons par transformer notre dataframe de ligne en un réseau :
...
plot(nets)
```
Cela commence à être bien, mais il reste un quelques liens isolés :
```{r}
plot(nets %>%activate(edges)%>% filter(st_length(x)<units::as_units(1000,"m")))
```
...
Il reste a découper cette longue ligne en tronçons de 200m pour se faire nous allons commencer par créer un ensemble de points distant de 200m le long de la ligne,et uniformiser l'échantillonage de la ligne :
```{r}
# On prend une ligne et on met un point tous les 100 mètres
points_eqd = line.geom  %>% st_line_sample(density = 1/10) %>% st_cast("POINT")
lines_eqd = line.geom  %>% st_line_sample(density = 1/10) %>% st_cast("LINESTRING")
split_points = points_eqd[seq(1,length(points_eqd),by=20)]
...
Le plus dur est fait. Il ne reste plus qu'a compter et à faire une carte :). Notez le `endCapStyle` sur le `st_buffer` pour ne pas rajouter de marge aux deux extrémitées des tronçons :
```{r}
...
ggplot(periph_count) + 
  ...
  labs(title = "Nombre de personnes accidentées sur le périphérique",
       subtitle = "en 2019, par portion de 200m",
       caption = "fichier BAAC 2019, ONISR\nantuki & comeetie, 2021",x="",y="")
