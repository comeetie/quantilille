...
Ce dernier exercice va s'appuyer sur les données des listings AirBnB à paris. Disponniblent sur le site [Inside AirBnB](http://insideairbnb.com/).
...
sel_metro = 
    c('Saint-Lazare','École Militaire','Saint-Germain-des-Prés','Rennes','Denfert-Rochereau','Colonel Fabien','Saint-Placide','Oberkampf',"Place d'Italie",'La Tour Maubourg','Les Halles','Filles du Calvaire','Mairie des Lilas','Porte de la Villette',"Porte d'Italie",'Montparnasse-Bienvenüe','Balard','Château Rouge',"Gare d'Austerlitz","Gare de l'Est",'Palais Royal - Musée du Louvre','Boulogne - Pont de Saint-Cloud','Richard Lenoir','Strasbourg - Saint-Denis','Gare de Lyon','Poissonnière','Tolbiac','Gare du Nord (Métro)')
...
```{r, echo=FALSE}
sel_metro=c('Saint-Lazare','École Militaire','Saint-Germain-des-Prés','Rennes','Denfert-Rochereau','Colonel Fabien','Saint-Placide','Oberkampf',"Place d'Italie",'La Tour Maubourg','Les Halles','Filles du Calvaire','Mairie des Lilas','Porte de la Villette',"Porte d'Italie",'Montparnasse-Bienvenüe','Balard','Château Rouge',"Gare d'Austerlitz","Gare de l'Est",'Palais Royal - Musée du Louvre','Boulogne - Pont de Saint-Cloud','Richard Lenoir','Strasbourg - Saint-Denis','Gare de Lyon','Poissonnière','Tolbiac','Gare du Nord (Métro)')

gg = subway.poly %>% st_join(listings.l93) %>% st_drop_geometry() %>% 
  filter(name.x %in% sel_metro, price<750) %>%  
  mutate(name.x=factor(name.x,levels=sel_metro))
ggplot(gg)+geom_boxplot(aes(y=name.x,x=price))+theme_bw()+labs(title="Répartition des prix d'une nuité",
       subtitle=" à proximité de certaines stations de métro ",caption = "fichier inside airBnB, ONISR\nantuki & comeetie, 2021", x="Prix d'une nuité ()",y="")
```
...
roads.geom <- st_read(dsn = "../lecture/data/osmdata/road.shp", quiet = TRUE)
river.geom <- st_read(dsn = "../lecture/data/osmdata/river.shp", quiet = TRUE)
