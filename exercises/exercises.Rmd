...
1. Faire une carte agrégée des iris de Paris (union simple)
2. Faire une zone tampon autour de 1km
3. Calculer les polygones de Voronoïs autour des accidents
4. Extraire les 50 premiers centroïdes de ces Voronoïs
5. Calculer la distance entre ces centroïdes
```
...
plot(st_geometry(iris.75.b), lwd=2, border = "red")
# 3. Calculer les polygones de Voronoïs autour des accidents
accidents.2019.paris.v <- st_collection_extract(
  st_voronoi(x = st_union(accidents.2019.paris)))
accidents.2019.paris.v <- st_intersection(accidents.2019.paris.v, st_union(iris.75))
accidents.2019.paris.v <- st_join(x = st_sf(accidents.2019.paris.v),
                                  y = accidents.2019.paris)
plot(st_geometry(accidents.2019.paris.v), col='lightblue', add=TRUE)
# 4. Extraire les 50 premiers centroïdes des Voronoïs
accidents.2019.paris.v.c <- st_centroid(accidents.2019.paris.v)[1:50,]
plot(st_geometry(accidents.2019.paris.v.c), add=TRUE, cex=1,
     col="red", pch=20)
plot(st_geometry(iris.75.u), border="blue", ltw=5, col=NA,
     add = TRUE)

# 5. distance entre centroïdes
mat <- st_distance(x=accidents.2019.paris.v.c,
                   y=accidents.2019.paris.v.c)
```
...
mapview(accidents.2019.paris %>%
        ...
        layer.name = "Gravité des accidents\nà Paris en 2019",
